...
| 1.6 | Invoice generation работает (Shopify Order Printer + Proforma темплейт) | 🔲 | US (установить app) + DS (темплейт) + CC | — |
| 1.7 | Discount tier применяется к ценам в каталоге и корзине | 🔲 | CC + DS | — |
| 1.8 | Topbar показывает {shop_name} · {tier} для залогиненных | 🔲 | CC | — |
| 1.9 | Apply for partnership форма работает (включая фикс phone field) | ❌ Blocked | US (Massy HTML) → DS → CC | BUGS #5 |
| 1.10 | Login / logout flow через Massy — работает на всех страницах | 🔲 | US (Massy config) + CC | — |
| 1.11 | Все 11 React-блоков мигрированы в Liquid | ✅ 8 of 11 exist / 🔲 audit + 3 Dawn-native | CC + DS | Аудит качества существующих + кастомизация header/footer/announcement-bar |
| 1.12 | Дашборд аккаунта (tabs + quick-reorder, без дублей) | 🔲 | CC | — |
| 1.13 | Локализация: убрать хардкод EN строк во всех B2B-секциях | 🔲 | CC + DS | BUGS #18, большая задача |

## Tier 2 — Желательно к запуску, но можно после

| # | Item | Status | Owner |
|---|------|--------|-------|
| 2.1 | Дашборд: вкладки orders + invoices работают, дубли удалены | 🔲 | CC |
